...
- (void)viewDidLoad
{
    ...
    self.locationDetail.imagePager.contentMode = UIViewContentModeScaleAspectFit;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.row == 0){
        ...
        if(cell == nil){
            ...
            cell.titleLbl.text = @"Jorge Diaz dice :";
            cell.contentLbl.text = @"Lo acabamos de ver la semana pasada en tlatelolco !!";
        }
        return cell;
    }
        ...
        if(cell == nil){
            cell = [TipCell tipCell];
            cell.titleLbl.text = @"Monica Pelaez dice :";
            cell.contentLbl.text = @"Le hemos visto la semana cera de la delegación GAM";
        }
        return cell;
    }
    else{
...
{
    return @[
             [UIImage imageNamed:@"1.jpg"],[UIImage imageNamed:@"2.jpg"],[UIImage imageNamed:@"3.jpg"],[UIImage imageNamed:@"4.jpg"]
             ];
}
...
{
    NSLog(@"Post action");
    UIAlertView* input = [[UIAlertView alloc]initWithTitle:@"Mandar Feedback" message:@"Cualquier dato que puedas aportar acerca de este caso nos " delegate:self cancelButtonTitle:@"Cancelar" otherButtonTitles:@"Mandar", nil];
    input.alertViewStyle = UIAlertViewStylePlainTextInput;
    
    [[input textFieldAtIndex:0] setDelegate:self];
    [[input textFieldAtIndex:0] resignFirstResponder];
    //just numers pad to prevent crashing
    [[input textFieldAtIndex:0] becomeFirstResponder];
    [input show];
